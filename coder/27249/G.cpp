...
struct Vec {
  ...
  int operator^(const Vec &rhs) const { return x * rhs.y - rhs.x * y; }
};
int sgn(int x) {
  if (x > 0) return 1;
  if (x < 0) return -1;
...
void solve() {
  ...
  for (int i = 0; i < q; i++) {
    ...
    int left1 = l[0] ^ (p - s[1]), left2 = l[1] ^ (p - s[2]),
        left3 = l[2] ^ (p - s[0]);

    debug(left1, left2, left3);

    if ((left1 == 0 && (s[0] - p) * (s[1] - p) <= 0) ||
        (left2 == 0 && (s[1] - p) * (s[2] - p) <= 0) ||
        (left3 == 0 && (s[2] - p) * (s[0] - p) <= 0)) {
      std::cout << "YES\n";
      continue;
    }
    ...
    if (sgn(left1) == sgn(left2) && sgn(left2) == sgn(left3)) {
      std::cout << "YES\n";
      continue;
    }
    std::cout << "NO\n";
