...
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/P8801
// Memory Limit: 128 MB
// Time Limit: 1000 ms
// 
...
void dfs(int idx, int inc, int dec, std::string num){
  int idx_value = num[idx] - '0';
  mmax(max, std::stoll(num));
  debug(idx, inc, dec, num);
  if(inc == 0 && dec == 0) return;
  if(idx >= num.size()) return;
  
  if((9 - idx_value) <= inc && (idx_value + 1) <= dec){
    num[idx] = '9';
    dfs(idx+1, inc - (9 - idx_value), dec, num);
    dfs(idx+1, inc, dec -  (idx_value+1), num);
  }else if((9 - idx_value) <= inc){
    num[idx] = '9';
    dfs(idx+1, inc - (9 - idx_value), dec, num);
  }else if((idx_value + 1) <= dec){
    num[idx] = '9';
    dfs(idx+1, inc, dec -  (idx_value+1), num);
  }else{
    std::string num2 = num;
    num2[idx] = (idx_value + inc) % 10 + '0';
    num[idx] = (idx_value - dec + 10) % 10 + '0';
    dfs(idx+1, 0, dec, num2);
    dfs(idx+1, inc, 0, num);
  }
}
