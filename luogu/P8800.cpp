...
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/P8800
// Memory Limit: 128 MB
// Time Limit: 1000 ms
// 
...
TO_STRING(Card){
  buffer << "(" << value.a << ", " << value.b << ", d=" << (value.b - value.a) << ")";
}

bool check(int k){
  int draw = 0;
  for(int i = 0; i < n; i++){
    if(c[i].a + c[i].b < k) return false;
    draw += std::max((int)0, (k - c[i].a));
  }
  return m > draw;
}
