// Problem: P3201 [HNOI2009] 梦幻布丁
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/P3201
// Memory Limit: 128 MB
// Time Limit: 1000 ms
//
...
void solve() {
  ...
  int ans = 0;// 颜色不相同的线段的数量
  rep1(i, n) {
    c[a[i]] = a[i]; // 颜色的变化情况
    ans += (a[i] != a[i - 1]);
    s[a[i]].push_back(i); // 这个颜色所在位置
  }
  while (m--) {
    int op;
    read(op);
    ...
    read(x, y);
    if (x == y) continue;
    // 选择颜色中数量少的，将现有颜色记为 c[x] 
    // 将 c[x] 变为 c[y] 色
    if (s[c[x]].size() > s[c[y]].size()) std::swap(c[x], c[y]);

    int cx = c[x]; 
    int cy = c[y];
    
    // 如果 x 颜色的左右有 y 颜色， 则将 x 变为 y 会使不同颜色的线段数目减一
    for (auto i : s[cx]) {
      if (a[i - 1] == cy) ans--;
      if (a[i + 1] == cy) ans--;
    }

    for (int i : s[cx]) a[i] = cy;
    for (int i : s[cx]) s[cy].push_back(i);
    s[cx].clear();
  }
}
