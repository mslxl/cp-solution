...
// clang-format off
#include <bits/stdc++.h> 
using i16 = short; using i32 = int; using i64 = long long; using u16= unsigned short; using u32 = unsigned i32; using u64 = unsigned long long;using f32 = float; using f64 = double; using f128 = long double;
...
i32 TESTCASE = 1; i32 main() { init(); for(i32 i = 1; i <= TESTCASE; i++){ solve(); if(i != TESTCASE) std::cout << "\n"; } return 0;}
...
template<const i32 T> struct MInt { const static i32 mod = T; i32 x; MInt(i32 x = 0) : x(x % mod) {} MInt(long long x) : x(i32(x % mod)) {} MInt(unsigned long long x): x(i32(x % mod)){} i32 val() { return x; } MInt operator + (const MInt &a) const { i32 x0 = x + a.x; return MInt(x0 < mod ? x0 : x0 - mod); } MInt operator - (const MInt &a) const { i32 x0 = x - a.x; return MInt(x0 < 0 ? x0 + mod : x0); } MInt operator * (const MInt &a) const { return MInt(1LL * x * a.x % mod); } MInt operator / (const MInt &a) const { return *this * a.inv(); } bool operator == (const MInt &a) const { return x == a.x; }; bool operator != (const MInt &a) const { return x != a.x; }; void operator += (const MInt &a) { x += a.x; if (x >= mod) x -= mod; } void operator -= (const MInt &a) { x -= a.x; if (x < 0) x += mod; } void operator *= (const MInt &a) { x = 1LL * x * a.x % mod; } void operator /= (const MInt &a) { *this = *this / a; } friend MInt operator + (i32 y, const MInt &a){ i32 x0 = y + a.x; return MInt(x0 < mod ? x0 : x0 - mod); } friend MInt operator - (i32 y, const MInt &a){ i32 x0 = y - a.x; return MInt(x0 < 0 ? x0 + mod : x0); } friend MInt operator * (i32 y, const MInt &a){ return MInt(1LL * y * a.x % mod);} friend MInt operator / (i32 y, const MInt &a){ return MInt(y) / a;} friend std::ostream &operator<<(std::ostream &os, const MInt &a) { return os << a.x;} friend std::istream &operator>>(std::istream &is, MInt &t){return is >> t.x;} MInt pow(int64_t n) const { MInt res(1), mul(x); while(n){ if (n & 1) res *= mul; mul *= mul; n >>= 1; } return res; } MInt inv() const { i32 a = x, b = mod, u = 1, v = 0; while (b) { i32 t = a / b; a -= t * b; std::swap(a, b); u -= t * v; std::swap(u, v); } if (u < 0) u += mod; return u; } };
#ifdef LOCAL
...
void solve() {
  ...
  V<V<V<Z>>> dp(2, V<V<Z>>(k + 1, V<Z>(n / k + 2, Z(0))));
  dp[0][0][0] = 1;
  ...
  for (i32 i = 0; i < n; i++) {
    for (i32 t = 0; t <= i / k; t++) {
      V<Z> tmp(k + 1, Z(0));
      for (i32 j = 0; j < k; j++) {
        tmp[j] = tmp[j] + dp[i % 2][j][t];

        if (j == k) continue;
        if (j == k - 1) {
          dp[(i + 1) % 2][0][t + 1] += dp[i % 2][j][t];
          continue;
        }
        dp[(i + 1) % 2][j + 1][t] += dp[i % 2][j][t] * (k - j);
      }
