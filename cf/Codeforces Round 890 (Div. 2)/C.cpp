...
// Institute URL: https://codeforces.com/contest/1856/problem/C Memory Limit:
...
void solve(const std::size_t testcase) {
  i32 n, k;
  read(n, k);
  std::vector<int> s(n + 1), pre(n + 1);
  reads(all1(s));
  // rep1(i, n) pre[i] = pre[i-1] + s[i];

  i64 L = *std::min_element(all1(s)), R = *std::max_element(all1(s)) + k + 1;

  auto check = [&](const i64 mid) -> bool {
    if (*std::max_element(all1(s)) >= mid) return true;
