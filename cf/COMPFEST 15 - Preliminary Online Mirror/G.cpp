...
// Rules, Teams Preferred) URL: https://codeforces.com/contest/1866/problem/G
// Memory Limit: 512 MB
// Time Limit: 3000 ms
//
...
void solve() {
  ...
  for (int i = 0; i < N; i++) {
    int L = mmax(0, i - D[i]);
    int R = mmin(N - 1, i + D[i]);
    seq[L].push_back({R, A[i]});
  }
  ...
  std::cout << fstTrue((int)0, (int)1e9, std::function<bool(int)>(check));
