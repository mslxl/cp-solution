...
void solve(const std::size_t testcase) {
  ...
  for (int i = 1; i <= n; i++) {
    if (i - lst[r[i]] == 1) {
      ...
      f[i][0] = std::max(f[lst[r[i]]][0], i - lst[r[i]] - 1);
      f[i][1] = std::min(std::max(f[lst[r[i]]][1], i - lst[r[i]] - 1),
                         std::max(f[lst[r[i]]][0], (i - lst[r[i]] - 1) / 2));
    }
    debug(i, r[i], lst[r[i]], f[i][0], f[i][1]);
    lst[r[i]] = i;
  }

  f[n + 1][0] = f[n + 1][1] = std::numeric_limits<int>::max();
  for (int i = 1; i <= k; i++) {
    f[n + 1][0] = std::min(f[n + 1][0], std::max(f[lst[i]][0], n - lst[i]));
    f[n + 1][1] = std::min(f[n + 1][1],
                           std::min(std::max(f[lst[i]][1], n - lst[i]),
                                    std::max(f[lst[i]][0], (n - lst[i]) / 2)));
  }

  std::cout << std::min(f[n + 1][0], f[n + 1][1]) << "\n";
}
