...
std::vector<pii> adjList[4000];
int color[4000];
bool bad = false;

void doDFS(int u, int c){
  if(color[u] != -1){
    if(color[u] != c) bad = true;
    return;
  }
  color[u] = c;
...
void solve(const std::size_t testcase) {
  ...
  rep(i, k) {
    ...
    read(x1,y1,x2,y2);
    x1--, y1--,x2--,y2--;
    adjList[std::min(x1,x2)].pb({n + std::min(y1,y2), (x1 + y1 != x2 + y2)});
    adjList[std::min(y1, y2) + n].pb({std::min(x1, x2), (x1 + y1 != x2 + y2)});
  }
  std::fill(color, color + 4000, -1);
  bad = false;
  
  for(int i = 0; i < n + m; i++){
    if(color[i] == -1) doDFS(i, 0);
  }
  std::cout << (bad ? "NO\n" : "YES\n");

  rep(i, n+m) adjList[i].clear();
}
