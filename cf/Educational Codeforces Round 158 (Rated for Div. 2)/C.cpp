...
template <typename A, typename BaseT> typename std::enable_if<std::is_integral<BaseT>::value, A>::type qpow(A a, BaseT n) { decltype(a) ans = 1; while (n) { if (n & 1) ans = ans * a; n >>= 1; a = a * a; } return ans; }
void YES(bool t = 1) { std::cout << (t? "YES" : "NO" ); } void NO(bool t = 1) { YES(!t); } void Yes(bool t = 1) { std::cout << (t? "Yes" : "No" ); } void No(bool t = 1) { Yes(!t); } void yes(bool t = 1) { std::cout << (t? "yes" : "no" ); } void no(bool t = 1) { yes(!t); }
#ifdef LOCAL
