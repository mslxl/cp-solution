...
void pushdown(int p) {
  auto &root = tr[p], &left = tr[lc], &right = tr[rc];
  if (root.f) {
    std::swap(left.sum0, left.sum1);
    std::swap(right.sum0, right.sum1);
    left.f ^= root.f, right.f ^= root.f, root.f = 0;
  }
}
...
int query(int p, int x, int y, int g) {
  if (x <= tr[p].l && tr[p].r <= y)
    if (g == 0)
      return tr[p].sum0;
    else
  ...
  pushdown(p);
  int res = 0;
  if (x <= m) res ^= query(lc, x, y, g);
  if (y > m) res ^= query(rc, x, y, g);
  return res;
}
