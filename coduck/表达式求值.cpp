...
void solve(const std::size_t testcase) {
  ...
  for(int i = 0; i < line.size(); i++){
    if(std::isdigit(line[i]) && std::isdigit(buff[buff.size()-1][0])){
      buff[buff.size()-1] += line[i];
    }else{
      buff.pb(std::string() + line[i]);
    }
  }
  debug(buff);
  ...
  for(int i = 0; i < buff.size(); ){
    if(buff[i] == "+"){
      ...
      i++;
      int a = std::atoi(buff[i].data());
      a *= st.top();
      st.pop();
      st.push(a % 10000);
      i++;
    } else {
      st.push(std::atoi(buff[i].data()));
      i++;
    }
  }
  int ans = 0;
