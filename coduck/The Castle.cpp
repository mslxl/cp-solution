...
int r[maxn][maxn];
bool vis[maxn][maxn];
int num_room, size_room;
using pii = std::pair<int, int>;

const int cup = 0b0010, cdown = 0b1000, cleft = 0b0001, cright = 0b0100;
...
void floodfill(int xx, int yy) {
  ...
  while (!q.empty()) {
    ...
    sz++;
    debug(x, y);
    if (!(r[y][x] & cup) && y > 1) q.push({x, y - 1});
    if (!(r[y][x] & cdown) && y < m) q.push({x, y + 1});
    if (!(r[y][x] & cleft) && x > 1) q.push({x - 1, y});
    if (!(r[y][x] & cright) && x < n) q.push({x + 1, y});
  }
  mmax(size_room, sz);
}
