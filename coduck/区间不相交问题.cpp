...
void solve(const std::size_t testcase) {
  ...
  read(n);
  std::vector<pii> s(n);
  rep(i, n) read(s[i].first, s[i].second);
  std::sort(all(s));

  int cnt = 0;
  int most_left = -1;
  int candidate_left = 0x3f3f3f, candidate = -1;
  int idx = 0;
  debug(s);
  while (idx != n) {
    for (int i = idx; i < n; i++) {
      if (s[i].first > most_left) {
        if (candidate_left > s[i].second) {
          candidate = i;
          candidate_left = s[i].second;
        }
      }
    }

    if(candidate_left > most_left){
      debug(idx, candidate);
      debug(s[idx], s[candidate]);
      most_left = candidate_left;
      idx = candidate;
      cnt++;
    }
    if(candidate_left == most_left){
