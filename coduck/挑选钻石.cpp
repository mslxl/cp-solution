...
struct Item {
  int cnt;
  int w, v;
  long double wpv() const { return ld(v) / w; }
  bool operator<(const Item &rhs) const { return wpv() > rhs.wpv(); }
  bool operator==(const Item &rhs) const {
    return std::abs(wpv() - rhs.wpv()) < 1e-8;
  }
};
...
int ans = 0;
void dfs(int i, int value, int weight) {

  ans = std::max(value, ans);
  if (i >= N || weight >= K)
    return;

  if (value + (K - weight) * items[i].wpv() <= ans)
    return;
  if (value + suf[i] <= ans)
    return;

  for (int cnt = items[i].cnt; cnt >= 0; cnt--) {
    if (weight + items[i].w * cnt > K)
      continue;
    dfs(i + 1, value + items[i].v * cnt, weight + items[i].w * cnt);
  }
}
...
void solve(const std::size_t testcase) {
  ...
  items.resize(N);
  suf.resize(N + 1);
  rep(i, N) read(items[i].w);
  rep(i, N) read(items[i].v);
  rep(i, N) items[i].cnt = i + 1;

  std::sort(all(items));
  ans = 0;
  for (int i = N - 1; i >= 0; i--)
    suf[i] = suf[i + 1] + items[i].cnt * items[i].v;
  dfs(0, 0, 0);
  std::cout << ans << "\n";
}
