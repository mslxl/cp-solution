...
void solve(const std::size_t testcase) {
  read(M, N);
  std::vector<int> W(N + 1), C(N + 1), P(N + 1);
  rep1(i, N) { read(W[i], C[i], P[i]); }
  ...
  for (int i = 1; i <= N; i++) {
    for (int j = 0; j <= M; j++) {
      ...
      if (P[i] == 0) {
        if (j >= W[i]) {
          f[i][j] = std::max(f[i][j], std::max(f[i - 1][j - W[i]] + C[i],
                                               f[i][j - W[i]] + C[i]));
        }
        ...
        for (int k = 0; k <= P[i]; k++) {
          if (j >= k * W[i]) {
            f[i][j] = std::max(f[i][j], f[i - 1][j - k * W[i]] + k * C[i]);
          }
        }
      }
    }
