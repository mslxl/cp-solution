...
void solve(const std::size_t testcase) {
  dayMax[1] = dayMax[3] = dayMax[5] = dayMax[7] = dayMax[8] = dayMax[10] = dayMax[12] = 31;
  dayMax[4] = dayMax[6] = dayMax[9] = dayMax[11] = 30;
  dayMax[2] = -1;
  
  int ans = 0;
  int s,e;
  read(s,e);
  for(int d = s; d <= e; d = nxtDay(d)){
      if(rev(d) == d){
        // std::cout << d << "\n";
        ans++;
      } 
