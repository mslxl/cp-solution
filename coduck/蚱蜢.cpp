...
std::set<char> vowel;
void solve(const std::size_t testcase) {
  vowel.insert('A');vowel.insert('E');
  vowel.insert('I');vowel.insert('O');
  vowel.insert('U');vowel.insert('Y');
  ...
  for(int i = 1; i < line.size(); i++){
    if(vowel.count(line[i])){
      min = std::max(min, i - last);
      last = i;
    }
  }
  min = std::max(min, int(line.size()) - last);
  std::cout << min;
}
