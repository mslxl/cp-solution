...
const int maxw = 200 + 17;
int f[maxn][maxw];
void solve(const std::size_t testcase) {
  int n, Wk;
  read(Wk, n);
  std::vector<int> W(n + 1), C(n + 1), N(n + 1);

  // reads(all(W)), reads(all(C));
  rep1(i, n) read(C[i], W[i], N[i]);
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = 0; j <= Wk; j++) {
      ...
      for (int k = 0; k <= N[i]; k++) {
        if (j >= C[i] * k) {
          f[i][j] = std::max(f[i - 1][j - C[i] * k] + W[i] * k, f[i][j]); // 看后面是 f[i][j]
        }
      }
    }
  }
