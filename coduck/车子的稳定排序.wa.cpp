...
int n, k;
bool mp[42][42];
bool cols[42], rows[42];
bool found = false;

void mark(int x, int y){
  rows[x] = cols[y] = mp[x][y] = true;
}

void unmark(int x, int y){
  rows[x] = cols[y] = mp[x][y] = false;
}

bool ismark(int x, int y){
  return mp[x][y] || rows[x-1] || rows[x+1] || rows[x] || cols[y-1] || cols[y] || cols[y+1];
}

void dfs(int x, int y, int w, int h, int rk){
  if(found) return;
  if(rk == 0){
    found = true;
    for(int i = 1; i <= n; i++){
  ...
  if(!ismark(x, y)){
    mark(x, y);
    dfs(x+2, y+2, w, h, rk-1);
    unmark(x, y);
  }
  
  if(!ismark(w, y)){
    mark(w, y);
    dfs(x, y+2, w-2, h, rk-1);
    unmark(w, y);
  }
  
  if(!ismark(x, h)){
    mark(x, h);
    dfs(x+2, y, w, h-2, rk-1);
    unmark(x, h);
  }
  
  if(!ismark(w, h)){
    mark(w, h);
    dfs(x, y, w-2, h-2, rk-1);
    unmark(w, h);
  }
}
