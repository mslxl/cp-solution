...
std::vector<int> add(std::vector<int> &a, std::vector<int> &b, int n) {
  ...
  for (int i = 0; i < std::max(a.size(), b.size()); i++) {
    int ia = i < a.size() ? a[i] : 0;
    int ib = i < b.size() ? b[i] : 0;
    int sum = ia + ib + inc;
    inc = sum / n;
    sum %= n;
    ans.pb(sum);
...
void solve(const std::size_t testcase) {
   ...
   for(char ch = '0'; ch <= '9'; ch++) mapping[ch] = ch-'0';
   for(char ch = 'A'; ch <= 'F'; ch++) {
     mapping[ch - ('A' - 'a')] = mapping[ch] = 10 + ch - 'A';
   }
   
   std::cin >> n >> inp;
   
   std::vector<int> a(inp.size());
   std::vector<int> b;
   for(int i = 0; i < inp.size(); i++) a[i] = mapping[inp[i]];
   b = a;
   std::reverse(all(b));
   if(a == b){
     std::cout << 0;
