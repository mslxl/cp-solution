...
int fa[200 + 17][200 + 17];
int fi[200 + 17][200 + 17];
int prefix[200 + 17];
...
void solve(const std::size_t testcase) {
  ...
  for (int len = 1; len <= n; len++) {
    for (int i = 1; i <= 2 * n; i++) {
      ...
      for (int s = i; s < j; s++) {
        fa[i][j] = std::max(
            fa[i][j], fa[i][s] + fa[s + 1][j] + (prefix[j] - prefix[i - 1]));

        fi[i][j] = std::min(fi[i][j], fi[i][s] + fi[s + 1][i + len] +
                                          (prefix[j] - prefix[i - 1]));
      }
      std::cerr << i << " " << j << " " << fa[i][j] << std::endl;
    }
  }

  int max = 0, min = std::numeric_limits<int>::max();
  for (int i = 1; i <= n; i++) {
    max = std::max(fa[i][i + n - 1], max);
    min = std::min(fi[i][i + n - 1], min);
  }
  std::cout << min << "\n" << max;
}
