// Problem: H - Local Minimum
// Contest: Virtual Judge - 狙击23网络赛#1
// URL: https://vjudge.net/contest/573471#problem/H
// Memory Limit: 512 MB
// Time Limit: 1000 ms
// 
...
void solve(const std::size_t testcase) {
  i32 n, m;
  read(n, m);
  std::vector<std::vector<i32>> M(n + 1, std::vector<i32>(m+1, 0));
  std::vector<i32> minInR(n+1, 0x3f3f3f3f), minInC(m+1, 0x3f3f3f3f);
  std::vector<i32> numInR(n+1, 0), numInC(m+1, 0);
  rep1(i, n){
    rep1(j, m){
      read(M[i][j]);
      minInR[i] = std::min(M[i][j], minInR[i]);
      minInC[j] = std::min(M[i][j], minInC[j]);
    }
  }
  ...
  rep1(i, n){
    rep1(j, m){
     if(M[i][j] == minInR[i]&& M[i][j] == minInC[j]) ans++; 
    }
  }
  std::cout << ans;
