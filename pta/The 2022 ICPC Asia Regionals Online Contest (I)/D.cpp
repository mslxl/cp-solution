...
void calc() {
  for (int len = 2; len <= 31; len++) {
    for (int tz = 1; tz <= mmin(16, len / 2); tz++) {
  
      V<int> bits(len);
      for (int i = tz; i < tz * 2; i++) {
        bits[i] = 1;
      }

      std::sort(bits.begin() + tz + 1, bits.end());
      do {
        int num = 0;
        for (int i = 0; i < len; i++) {
          if (bits[i]) {
        ...
        good.pb(num);
      } while (std::next_permutation(bits.begin() + tz + 1, bits.end()));
    }
  }
  std::sort(all(good));
  good.erase(std::unique(all(good)), good.end());
}

void solve() { 
  int l, r;
  read(l, r);
  auto lo = std::lower_bound(all(good), l);
  auto ho = std::upper_bound(all(good), r);
  if(lo == ho){
    std::cout << -1;
  }else{
    std::cout << *lo;
