...
void solve() {
  ...
  V<V<bool>> ban('z' + 1, V<bool>('z' + 1, false));
  V<bool> app('z' + 1, false);
  ...
  V<V<int>> f(s.size() + 1, V<int>('z' + 1, 0));
  for (int i = 1; i < s.size(); i++) f[i][s[i]] = 1;
  ...
  for (int i = 1; i < s.size(); i++) {
    if (!app[s[i]]) {
      ...
      for (char j = 'a'; j <= 'z'; j++) {
        f[i][j] = mmax(f[i][j], f[i-1][j]);
        if (!ban[j][s[i]]) f[i][s[i]] = mmax(f[i][s[i]], f[i - 1][j] + 1);
      }
    }
  }
  // debug(V<int>(f[4].begin() + 'a', f[4].end()));

  int ans = 0;

  for (char i = 'a'; i <= 'z'; i++) {
    ans = mmax(ans, f[s.size() - 1][i]);
  }
