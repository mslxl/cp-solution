// Problem: 吉吉与福桔黄的故事
// Contest: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/70306/B
...
int n, m, k;
int f[NN];
std::vector<int> x(NN + 1), v(NN + 1), w(NN + 1), sum1(NN + 1), sum2(NN + 1);

struct Node {
  int l, r, sum;
} s[NN];

int query_prefix1(int l, int r) { return sum1[r] - sum1[l - 1]; }

int query_prefix2(int l, int r) { return sum2[r] - sum2[l - 1]; }

int check(int l, int r) { return (x[r] - x[l]) << 1; }
void solve() {
  read(n, m, k);
  rep1(i, n) read(x[i]);
  rep1(i, n) read(v[i]);
  ...
  std::sort(s + 1, s + pos + 1, cmp);
  for (int k = 1, pre = -1; k <= pos; k++) {
    if (s[k].sum >= query_prefix2(s[k].l, s[k].r)) {
      ans = mmax(ans, query_prefix1(s[k].l, s[k].r));
      continue;
    }
    if (query_prefix1(s[k].l, s[k].r) <= ans) continue;
    if (pre == s[k].l) {
      for (int j = s[k].sum; j >= v[s[k].r]; j--) {
        f[j] = mmax(f[j], f[j - v[s[k].r]] + w[s[k].r]);
      }
    } else {
      std::fill(std::begin(f), std::end(f), 0);
      for (int i = s[k].l; i <= s[k].r; i++)
        for (int j = s[k].sum; j >= v[i]; j--) {
          f[j] = mmax(f[j], f[j - v[i]] + w[i]);
        }
    }
    ans = mmax(ans, f[s[k].sum]);
    pre = s[k].l;
  }
  cout << ans << "\n";
}
