// Problem: 石子游戏
// Contest: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/70306/F
...
std::multiset<int> b;
/*
考虑从高位往低位插入 01 trie 后的结构，
异或的最小值一定产生在 trie 树 dfs 序相邻的两数之间，
而这个 dfs 序就是 sg 值的大小。

因此 set 维护前驱后继，再利用另一个数据结构维护最小值即可。
...
void add(int v){
  if(cnt[v] == 0){
    ...
    if(*a.rbegin() != v) { // dfs 后一个相邻
      y = *std::next(a.find(v));
      b.insert(y ^ v);
    }
    if(x !=-1 && y != -1){
      // ?
      // 大概是插入导致 dfs 序中的原本相邻相不再相邻
      b.erase(b.find(x ^ y));
    }
  }
  cnt[v]++;
...
void del(int v){
  ...
  if(cnt[v] == 0){
    ...
    if(x != -1 && y != -1){
      b.insert(x ^ y); // 删除 v 后，原本 trie 中 dfs 序会出现重现相邻的
    }
    a.erase(v);
  }
}
...
void solve() {
  ...
  rep(tt, q){
    ...
    read(op);
    if(op == 3){
      if(b.empty()) std::cout << (k+1) << "\n";
      else std::cout << (*b.begin()) << "\n";
    }else{
      int val;
      read(val);
      if(op == 1) add(val % (k+1));
