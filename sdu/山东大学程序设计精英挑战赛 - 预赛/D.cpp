// Problem: 五子棋
// Contest: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/70306/D
...
void solve() {
  ...
  auto ck_x = [&](int x, int y) {
    if (x + 4 <= 15) {
      int n = 0;
      for (int i = x; i <= x + 4; i++) {
        if (m[i][y] == '1') n++;
        if (m[i][y] == '2') return false;
      }
      return n >= 4;
    } else {
      return false;
  ...
  auto ck_y = [&](int x, int y) {
    if (y + 4 <= 15) {
      int n = 0;
      for (int i = y; i <= y + 4; i++) {
        if (m[x][i] == '1') n++;
        if (m[x][i] == '2') return false;
      }
      return n >= 4;
    } else {
      return false;
  ...
  auto ck_vx = [&](int x, int y) {
    if (x + 4 <= 15 && y + 4 <= 15) {
      int n = 0;
      for (int i = 0; i <= 4; i++) {
        if (m[x + i][y + i] == '1') n++;
        if (m[x + i][y + i] == '2') return false;
      }
      return n >= 4;
    } else {
      return false;
  ...
  for (int i = 1; i <= 15; i++) {
    for (int j = 1; j <= 15; j++) {
      if (ck_x(i, j) || ck_y(i, j) || ck_vx(i, j) || ck_vy(i, j)) {
        std::cout << "Yes";
        return;
      }
    }
