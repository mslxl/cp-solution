// Problem: [HNOI2007]分裂游戏
// Contest: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/34655/H
...
int mex(int v){
  ...
  for(int i = v+1; i < n; i++){
    for(int j = i; j < n; j++){
      int sg_value = sg[i] ^ sg[j];
      mex_vis[sg_value] = true;
    }
  }
...
void solve(const std::size_t testcase) {
  ...
  rep(i, n) if(p[i] % 2 == 1) ans ^= sg[i]; // 作 SG和, 即题目中的 SG
  ...
  for(int i = 0; i < n; i++){
    ...
    for(int j = i + 1; j < n; j++){
      for(int k = j; k < n; k++){
        if((ans^ sg[i] ^ sg[j] ^ sg[k]) == 0){ // sg[i] ^ sg[j] ^ sg[k] 实际是一次游戏(单个的 sg 和),一方行动后另一方必败
           sol++;
           if(sol == 1){
             std::cout << i << " " << j << " "<< k << "\n";
           }
